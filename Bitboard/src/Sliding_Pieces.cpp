...
std::array<uint64_t, Sliding_Pieces::MAX_NUM_ROOK_MOVES> Sliding_Pieces::all_rook_moves(uint64_t bitboard, uint64_t empty_squares, uint64_t enemy_squares) {
    ...
    for(int i=move_index; i<MAX_NUM_ROOK_MOVES; i++) {
...
std::array<uint64_t, Sliding_Pieces::MAX_NUM_BISHOP_MOVES> Sliding_Pieces::all_bishop_moves(uint64_t bitboard, uint64_t empty_squares, uint64_t enemy_squares) {
  ...
  //make all other indices empty bitboards
  for(int i=move_index; i<MAX_NUM_BISHOP_MOVES; i++) {
      unique_post_move_bitboards[i] = 0x0;
  }
...
uint64_t Sliding_Pieces::get_vertical_moves(uint64_t bitboard, uint64_t empty_squares, uint64_t enemy_squares) {
    ...
    uint64_t diagonal_bitboard = Bitboard::flip_diagonal(bitboard);
    uint64_t diagonal_empty_squares = Bitboard::flip_diagonal(empty_squares);
    uint64_t diagonal_enemy_squares = Bitboard::flip_diagonal(enemy_squares);

    uint64_t up_ray = Sliding_Pieces::get_left_horizontal_ray(diagonal_bitboard, diagonal_empty_squares, diagonal_enemy_squares);
    uint64_t down_ray = Sliding_Pieces::get_right_horizontal_ray(diagonal_bitboard, diagonal_empty_squares, diagonal_enemy_squares);

    uint64_t to_ret = Bitboard::flip_diagonal(up_ray | down_ray);
